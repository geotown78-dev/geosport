...
export default function AdminPage({ user }: { user: any }) {
  ...
  async function switchSource(type: "camera" | "screen") {
    if (!isBroadcasting || !activeStream) return;
      ...
      if (type === "screen") {
        ...
        if (newStream.getAudioTracks().length === 0) {
          try {
            ...
            micStream.getAudioTracks().forEach(track => newStream.addTrack(track));
          } catch(e) {}
        }
      } else {
  ...
  async function startBroadcasting(stream: Stream, type: "camera" | "screen" = "camera") {
    try {
      ...
      alert("წდომა ვერ მოხერხდა. აუცილებელია ეკრანის გაზიარების ნებართვა.");
      console.error(err);
    }
  }
              ...
              <div className="absolute bottom-10 left-1/2 -translate-x-1/2 w-full max-w-xl px-4">
                <div className="bg-black/60 backdrop-blur-3xl border border-white/10 p-2 rounded-2xl flex items-center justify-between shadow-2xl">
                  ...
                  <div className="flex items-center gap-4 px-4">
                    <button onClick={() => { if(streamRef.current) { streamRef.current.getAudioTracks()[0].enabled = !isMicOn; setIsMicOn(!isMicOn); }}} className={cn("p-3 rounded-full transition-all", isMicOn ? "text-slate-400 hover:text-white hover:bg-white/10" : "bg-red-600 text-white shadow-lg shadow-red-900/40")}>
